...
export class Game {
    ...
    private _constructAStarGrid(): number[][] {
        ...
        relevantElements.forEach((child) => {
            const x = Math.min(Game._sceneToGrid(child.position.x), PROPERTIES.GRID_WIDTH - 1);
            const z = Math.min(Game._sceneToGrid(child.position.z), PROPERTIES.GRID_HEIGHT - 1);
            grid[z][x] = 1;
        });
